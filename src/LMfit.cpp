...
void LMfit::autoiter()
{
    ...
    fp stop_rel = F->get_settings()->lm_stop_rel_change;
    fp max_lambda = F->get_settings()->lm_max_lambda;
    if (stop_rel > 0) {
    ...
    //pre: init() callled
{
    ...
    if (chi2_ < chi2) { // better fitting
        ...
        a = beta_;
        compute_derivatives(a, dmdm_, alpha, beta);
        lambda /= F->get_settings()->lm_lambda_down_factor;
        return true;
    }
    else {// worse fitting
        lambda *= F->get_settings()->lm_lambda_up_factor;
        return false;
    }
}
